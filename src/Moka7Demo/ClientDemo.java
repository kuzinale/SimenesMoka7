...
//	 If MakeAllTests = true, also DBWrite and Run/Stop tests will be performed
    ...
    private static String IpAddress = "192.168.0.10";
    ...
    {
    	TestBegin("ReadArea()");
    	int Result = Client.ReadArea(S7.S7AreaDB, DBSample, 20, DataToMove, Buffer);
        if (Result==0)
        {
        	System.out.println("DB "+DBSample+" succesfully read using size reported by DBGet()");
        }
        for (int i = 20; i < DataToMove; i+=4) {
            System.out.println(S7.GetFloatAt(Buffer,i));
        }
    ...
    {
    	TestBegin("WriteArea()");
    	int Result = Client.WriteArea(S7.S7AreaDB, DBSample, 60, DataToMove, Buffer);
        if (Result==0)
        {
